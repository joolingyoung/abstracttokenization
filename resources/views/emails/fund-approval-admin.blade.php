...
<tr class='email-body'>
    <td colspan='2'>
        <h2>Fund Details</h2>
        @if (isset( $data['fund-images'] ))
            @foreach ($data['fund-images'] as $item)
                <img src="{{$item['src']}}" class='sponsor' alt='Fund-Logo'>
            @endforeach
        @endif
        <br />Fund Name: {{ isset($data['fund-name']) ? $data['fund-name'] : '' }}
        <br />Fund Address: {{ isset($data['fund-address']) ? $data['fund-address'] : '' }}
        <br />City: {{ isset($data['city']) ? $data['city'] : '' }}
        <br />State: {{ isset($data['state']) ? $data['state'] : '' }}
        <br />ZipCode: {{ isset($data['zip']) ? $data['zip'] : '' }}
        <br />Country: {{ isset($data['country']) ? $data['country'] : '' }}
        <br />Opportunity Type: {{ isset($data['opportunity-type']) ? $data['opportunity-type'] : '' }}
        <br />Type of Fund: {{ isset($data['type-of-fund']) ? $data['type-of-fund'] : '' }}
        <br />If Open Eneded, where is the capital from?: {{ isset($data['capital-origin']) ? $data['capital-origin'] : '' }}
        <br />Capital Deployed: {{ isset($data['capital-deployed']) ? $data['capital-deployed'] : '' }}
        <br />Provide Fund description, strategy and asset type / market focus:
                {{ isset($data['fund-description']) ? $data['fund-description'] : '' }}
        <br />Vacancy Rate: {{ isset($data['vacancy-rate']) ? $data['vacancy-rate'] : '' }}
        <br />Proforma/Current NOI: {{ isset($data['proforma-current-noi']) ? $data['proforma-current-noi'] : '' }}
        <br />Annual Cash Flow: {{ isset($data['annual-cash-flow']) ? $data['annual-cash-flow'] : '' }}
        <br />1031 Exchange: {{ isset($data['1031-exchange']) ? $data['1031-exchange'] : '' }}
        <br />Market Value: {{ isset($data['market-value']) ? $data['market-value'] : '' }}
        <br />Square Footage: {{ isset($data['square-footage']) ? $data['square-footage'] : '' }}
        <br />Property Class: {{ isset($data['property-class']) ? $data['property-class'] : '' }}
        <br />Total Debt: {{ isset($data['total-debt']) ? $data['total-debt'] : '' }}
        <br />Payoff Date: {{ isset($data['payoff-date']) ? $data['payoff-date'] : '' }}
        <br />Amortizing
                <?php
...
<tr class='email-body'>
    <td colspan='2'>
        ...
        <br />Minimum Raise Amount: {{ isset($data['minimum-raise-amount']) ? $data['minimum-raise-amount'] : '' }}
        <br />Maximum Raise Amount: {{ isset($data['maximum-raise-amount']) ? $data['maximum-raise-amount'] : '' }}
    </td>
</tr>
<tr class='email-body'>
    <td colspan='2'>
...
<tr class="email-button">
    <td>
        <a href="{{$host}}/fund/approve?id=<?= $data['id']?>&token=<?= $data['approval_token']?>" style="min-width:180px;" class="button approve">Approve</a>
    </td>
    <td>
        <a href="{{$host}}/fund/reject?id=<?= $data['id']?>&token=<?= $data['approval_token']?>" class="button reject">Reject</a>
    </td>
</tr>
@endsection
